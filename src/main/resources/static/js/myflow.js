(function ($) {
    ...
    myflow.rect = function (o, r, id) {
        var _this = this,
            _uid = myflow.util.nextId().toString(),
            _id = id || _uid,
            ...
            image_suffix = '.png',
            _ox, _oy; // 拖动时，保存起点位置;
        _o.text.text = _o.type + _uid;
        _rect = _r.rect(_o.attr.x, _o.attr.y, _o.attr.width, _o.attr.height, _o.attr.r).hide().attr(_o.attr);
        _img = _r.image(myflow.config.basePath + _o.type + image_suffix, _o.attr.x, _o.attr.y, _o.img.width, _o.img.height).hide();
        _name = _r.text(_o.attr.x, _o.attr.y, _o.name.text).hide().attr(_o.name);
        _text = _r.text(_o.attr.x, _o.attr.y, _o.text.text).hide().attr(_o.text); // 文本
        ...
        $([_rect.node, _text.node, _name.node, _img.node]).bind('click', function () {
            ...
            switch (mod) {
                ...
                case 'path':
                    ...
                    // alert(pre.getId());
                    // 所点击的节点不为空、不是之前的节点，并且所选节点是ID是RECT+ID的节点，刚触放添加path事件
                    var paths = myflow.config.tempData.paths,
                        flag = false;
                    for (var k in paths) {
                        if (paths[k]) {
                            if (pre && pre.getId() == paths[k].from().getId() && _this.getId() == paths[k].to().getId()) {
                                flag = true;
                                break;
                            }
                        }
            ...
            $(_r).data('currNode', _this);
            //传递参数
            var nodeOption = $.extend(true, {}, vue.nodeMap[_o.type], _o.props);
            _o.props = nodeOption;
            var propList = [];
            var ruleList = [];
            var lanList = [];
            ...
            if (nodeOption.prop) {
                nodeOption.prop.id = _this.getId();
                //打开dialog,从set里删除
                vue.rectIdSet.delete(nodeOption.prop.id);
                nodeOption.prop.name = _text.attrs.text;
                for (var key in nodeOption.prop) {
                    propList.push({
                        key: key,
                        value: {
                            value: nodeOption.prop[key],
                            editFlag: false
                        }
                    });
                }
            ...
            if (nodeOption.rule) {
                nodeOption.rule.forEach(function (item) {
                    ruleList.push({
                        index: item.index,
                        type: item.type,
                        fromNodeId: {
            ...
            if (nodeOption.lan) {
                nodeOption.lan.forEach(function (item) {
                    lanList.push({
                        type: item.type,
                        language: {
                            value: item.language,
            ...
            vue.prop.propData = propList;
            vue.rule.ruleData = ruleList;
            vue.lan.lanData = lanList;
        ...
        function getBoxPathString() {
            return 'M' + _bbox.x + ' ' + _bbox.y + 'L' + _bbox.x + ' ' + (_bbox.y + _bbox.height) + 'L' + (_bbox.x + _bbox.width) + ' ' + (_bbox.y + _bbox.height) + 'L' + (_bbox.x
                + _bbox.width) + ' ' + _bbox.y + 'L' + _bbox.x + ' ' + _bbox.y;
        }
        ...
        function resize() {
            ...
            switch (_o.showType) {
                ...
                case 'image&text':
                    ...
                    _text.attr({
                        x: rx + _o.img.width + rw / 2 - _o.img.width,
                        y: ry + rh / 2 + myflow.config.lineHeight
                    }).show(); // 文本
                    _img.attr({
                        x: rx + 2,//调整位置
                        y: ry + (rh - _o.img.height) / 2
    ...
    myflow.path = function (o, r, from, to, guid, ec, dots, id) {
        var _this = this,
            ...
            _path, _markpath, _arrow, _text, _textPos = _o.text.textPos,
            _ox, _oy, _from = from,
            _to = to,
            _id = id || 'path' + myflow.util.pathId(),
            _dotList, _autoText = true;
        _o.lineID = guid;
        var oec = (ec > 0 ? (parseInt(ec) == 1 ? 25 : parseInt(ec) * 9 + 22) : 0);
        ...
        _markpath = _r.path(_o.attr.path.path).attr({
            fill: "none",
            stroke: "white",
        ...
        _text = _r.text(0, 0, _o.text.text || _o.text.patten.replace('{from}', _from.text()).replace('{to}', _to.text())).attr(_o.attr.text);
        _text.drag(function (dx, dy) {
            if (!myflow.config.editable) {
                return;
            }
        ...
        this.toJson = function () {
            var data = {
                from: _from.getProps().prop ? _from.getProps().prop.id : _from.getId(),
                to: _to.getProps().prop ? _to.getProps().prop.id : _to.getId(),
    ...
    myflow.init = function (c, o) {
        ...
        // 添加状态
        var addRectHandler = function (e, type, o) {
            var data = getJson();
            var rect = new myflow.rect($.extend(true, myflow.config.rect, {
                type: type
            }, o), _r);

            if (_states[rect.getId()]) { //当前id已经存在
                var id = myflow.util.nextId();
                while (_states[id]) {
                    id = myflow.util.nextId();
                }
                rect.setId(id);
            }

            vue.rectIdSet.add(rect.getId().toString());
        ...
        $(_r).bind('addrect', addRectHandler);
        ...
        var addpathHandler = function (e, from, to, dots) {
            ...
            if (_paths[path.getId()]) { //当前id已经存在
                var id = 'path' + myflow.util.pathId();
                while (_paths[id]) {
                    id = 'path' + myflow.util.pathId();
                }
                path.setId(id);
            }
        ...
        $(_r).bind('restore', function () {
            //清空数据
            _r.clear();
            vue.rectIdSet = new Set();
            _states = {}, _paths = {};

            var flowData = vue.flow;
            if (flowData) {
                restore(flowData);
                myflow.config.tools.restoreSuccess();
            }
        });
        ...
        if (myflow.config.editable) {
            ...
            $(c).droppable({
                ...
                drop: function (event, ui) {
                    $(_r).trigger('addrect', [ui.helper.attr('type'), {
                        attr: {
                            x: ui.helper.offset().left + 10,
                            y: ui.helper.offset().top - 60
                        }
                    }]); // 10和60调整偏移量
                }
            });
            ...
            function getJson() {
                ...
                for (var k in _states) {
                    if (_states[k]) {
                        data.states[k] = _states[k].toJson();
                    }
                }
                data.paths = {};
                for (var k in _paths) {
                    if (_paths[k]) {
                        data.paths[k] = _paths[k].toJson();
                    }
                }
                return data;
            }
        ...
        function restore(data) {
            ...
            if (data.paths) {
                for (var k in data.paths) {
                    var from = rmap && rmap[data.paths[k].from];
                    var to = rmap && rmap[data.paths[k].to];
